...
def gerar_prompt(str_palavras_certas, str_palavras_erradas, str_enderecos_coordenadas):
    prompt = """
    Você receberá imagens extraídas de stories do Instagram de casas de eventos. Elas contêm ou não flyers com informações sobre festas, artistas e programações que normalmente são postados semanalmente. 
    Caso alguma imagem não contenha informações relevantes sobre eventos descarte-a, se não contiver um evento com titulo nem data, é um forte candidato a não ser o que estamos procurando. 
    ...
    Extraia todas as informações com máxima precisão. Se necessário, pesquise na internet o endereço e Instagram do evento ou casa do evento. A data e hora do evento são obrigatórias.     
    Use o nome da imagem com o numero da imagem que melhor usou para ler o evento (como por exemplo: "story_[numero].png" substituindo [numero] pelo número real da imagem que continha o evento) para preencher o campo flyer_imagem.
    ...
    Os campos de titulo e data_evento são obrigatórios, para ser um evento válido.
